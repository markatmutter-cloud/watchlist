#!/usr/bin/env python3
"""
Bulang & Sons scraper — Shopify, EUR-priced.

Scopes to the /collections/watches-for-sale collection (their curated
in-stock list) rather than the full /products.json which includes
straps, accessories and tools.

Run: python3 bulangandsons_scraper.py
Output: bulangandsons_listings.csv
"""
...
COLLECTION_PATH = "/collections/watches-for-sale"
...
def detect_brand(name, vendor=""):
    if vendor and vendor not in ("Bulang and Sons", "Bulang & Sons", ""):
        for b in BRANDS:
...
def parse_product(p):
    ...
    published_at = (p.get("published_at") or "2026-04-19")[:10]
    handle = p.get("handle", "")
...
def main():
    print("Fetching Bulang & Sons watches-for-sale collection...")
    ...
    out = "bulangandsons_listings.csv"
