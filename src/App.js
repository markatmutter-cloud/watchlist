    ...
    <div style={{ background: "var(--card-bg)", display: "flex", flexDirection: "column", position: "relative" }}>
      <a href={item.url} target="_blank" rel="noopener noreferrer"
        ...
        <div style={{ position: "relative", paddingTop: "100%", overflow: "hidden" }}>
          ...
          {isNew && (
            <div style={{ position: "absolute", top: 6, left: 6, background: "rgba(24,95,165,0.92)", color: "#fff", fontSize: 8, padding: "2px 6px", borderRadius: 8, letterSpacing: "0.06em", fontWeight: 600 }}>NEW</div>
          )}
          {item.currency && item.currency !== "USD" && (
            <div style={{ position: "absolute", bottom: 6, left: 6, background: "rgba(0,0,0,0.5)", color: "#fff", fontSize: 8, padding: "2px 5px", borderRadius: 5 }}>{item.currency}</div>
          )}
        </div>
...
function Chip({ label, active, onClick, blue, count }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? "var(--text1)" : blue ? "transparent" : "transparent",
...
export default function Dial() {
  ...
  const cols = isMobile ? 3 : isDesktop ? 5 : 3;
  ...
  useEffect(() => { setPage(1); }, [filterSources, filterBrands, search, sort, maxPos, statusFilter, newDays]);
  ...
  const newCounts = useMemo(() => {
    ...
    return { 1: fs.filter(i => daysAgo(i.date) <= 1).length, 3: fs.filter(i => daysAgo(i.date) <= 3).length, 7: fs.filter(i => daysAgo(i.date) <= 7).length, 30: fs.filter(i => daysAgo(i.date) <= 30).length };
  }, [items]);
  ...
  const allFiltered = useMemo(() => {
    ...
    return its;
  }, [items, filterSources, filterBrands, search, sort, priceMax, statusFilter, newDays]);
  ...
  const hasFilters = filterSources.length > 0 || filterBrands.length > 0 || search || maxPos < 100 || statusFilter !== "for_sale" || newDays > 0;
  const resetFilters = () => { setFilterSources([]); setFilterBrands([]); setSearch(""); setMaxPos(100); setStatusFilter("for_sale"); setNewDays(0); };

  const visibleBrands = brandsExpanded ? BRANDS : BRANDS.slice(0, BRANDS_SHOW);
  const NEW_OPTS = [{ label: "Today", days: 1 }, { label: "3 days", days: 3 }, { label: "This week", days: 7 }, { label: "This month", days: 30 }];

  const baseStyle = { fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Text','Helvetica Neue',sans-serif", WebkitFontSmoothing: "antialiased", minHeight: "100vh", background: "var(--bg)", color: "var(--text1)", ...Object.fromEntries(Object.entries(c).map(([k, v]) => [k, v])) };
  ...
  const FilterPanel = ({ inDrawer }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      {/* Status */}
      <div style={{ padding: inDrawer ? "14px 16px 10px" : "12px 16px 8px" }}>
        <div style={{ fontSize: 9, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text3)", marginBottom: 8 }}>View</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {[["for_sale", "For sale", forSaleCount], ["sold", "Sold", items.filter(i=>i.sold).length]].map(([val, label, count]) => (
            <button key={val} onClick={() => setStatusFilter(val)} style={{
              ...
              color: statusFilter === val ? "var(--text1)" : "var(--text2)",
          ...
          <button onClick={() => setTab("watchlist")} style={{
            ...
            padding: "6px 8px", borderRadius: 8, border: "none", cursor: "pointer",
            fontFamily: "inherit", fontSize: 13,
            background: tab === "watchlist" ? "var(--surface)" : "transparent",
            color: tab === "watchlist" ? "var(--text1)" : "var(--text2)",
            fontWeight: tab === "watchlist" ? 500 : 400,
          }}>
            <span>Watchlist</span>
      ...
      <div style={{ padding: "12px 16px 8px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          ...
          {BRANDS.length > BRANDS_SHOW && (
            <SidebarChip label={brandsExpanded ? "Less ↑" : `+${BRANDS.length - BRANDS_SHOW} more`} active={false} onClick={() => setBrandsExpanded(!brandsExpanded)} blue />
          )}
        </div>
      </div>
      <div style={{ height: "0.5px", background: "var(--border)", margin: "0 12px" }} />
      ...
      {/* Sort */}
      <div style={{ padding: "12px 16px 10px" }}>
        <div style={{ fontSize: 9, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text3)", marginBottom: 8 }}>Sort</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {[["date", "Newest first"], ["price-asc", "Price: low to high"], ["price-desc", "Price: high to low"]].map(([val, label]) => (
            <button key={val} onClick={() => setSort(val)} style={{
              ...
              color: sort === val ? "var(--text1)" : "var(--text2)",
      ...
      {hasFilters && (
        <>
          ...
          <div style={{ padding: "10px 16px" }}>
            <button onClick={resetFilters} style={{ fontSize: 12, color: "#185FA5", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "inherit" }}>
              Reset all filters
            </button>
          </div>
        </>
    ...
    <>
      <div style={{ ...gridStyle, borderRadius: 10, overflow: "hidden" }}>
        ...
        {allFiltered.length === 0 && (
          <div style={{ gridColumn: "1/-1", padding: 48, textAlign: "center", color: "var(--text3)", fontSize: 14 }}>No watches match your filters</div>
        )}
      </div>
        ...
        <div style={{ ...gridStyle, borderRadius: 10, overflow: "hidden" }}>
          {watchItems.map(item => (
            <Card key={item.id}
              ...
              wished={true} onWish={handleWish} compact={compact} />
          ))}
        </div>
      ...
      <div style={baseStyle}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px 8px", borderBottom: "0.5px solid var(--border)" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setDrawerOpen(true)} style={{
              width: 32, height: 32, borderRadius: "50%", border: "0.5px solid var(--border)",
              background: hasFilters ? "var(--text1)" : "var(--surface)",
              color: hasFilters ? "var(--bg)" : "var(--text2)",
              cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center"
            }}>
              <FilterIcon />
            </button>
            <button onClick={() => setDark(!dark)} style={{ width: 32, height: 32, borderRadius: "50%", border: "0.5px solid var(--border)", background: "var(--surface)", color: "var(--text2)", cursor: "pointer", fontSize: 13 }}>
              {dark ? "☀" : "◑"}
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: "8px 14px", borderBottom: "0.5px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--surface)", borderRadius: 10, padding: "8px 12px" }}>
            <SearchIcon />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reference or brand..."
              style={{ flex: 1, border: "none", background: "transparent", fontSize: 14, color: "var(--text1)", outline: "none", fontFamily: "inherit" }} />
          </div>
        </div>
        ...
        {drawerOpen && (
          <div style={{ position: "fixed", inset: 0, zIndex: 100 }}>
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "var(--bg)", borderRadius: "16px 16px 0 0", maxHeight: "85vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px 10px", borderBottom: "0.5px solid var(--border)" }}>
                <span style={{ fontSize: 15, fontWeight: 500 }}>Filters</span>
                <button onClick={() => setDrawerOpen(false)} style={{ fontSize: 22, background: "none", border: "none", color: "var(--text2)", cursor: "pointer", lineHeight: 1 }}>×</button>
              </div>
              <FilterPanel inDrawer />
              <div style={{ padding: "12px 16px 24px", borderTop: "0.5px solid var(--border)" }}>
  ...
  return (
    <div style={{ ...baseStyle, display: "flex", height: "100vh", overflow: "hidden" }}>
      ...
      {/* Sidebar */}
      <div style={{ width: 210, flexShrink: 0, borderRight: "0.5px solid var(--border)", overflowY: "auto", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "10px 16px 14px", borderTop: "0.5px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 10, color: "var(--text3)" }}>Updated twice daily</span>
          <button onClick={() => setDark(!dark)} style={{ fontSize: 13, background: "none", border: "none", cursor: "pointer", color: "var(--text2)" }}>
            {dark ? "☀" : "◑"}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: "0.5px solid var(--border)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, background: "var(--surface)", borderRadius: 8, padding: "7px 12px" }}>
            <SearchIcon />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reference or brand..."
              style={{ flex: 1, border: "none", background: "transparent", fontSize: 13, color: "var(--text1)", outline: "none", fontFamily: "inherit" }} />
          </div>
