#!/usr/bin/env python3
"""
Watch Brothers London scraper - Squarespace Commerce items[] API.
Run: python3 watchbrotherslondon_scraper.py
Requires: pip install requests
Output: watchbrotherslondon_listings.csv

Squarespace exposes any collection as JSON by appending ?format=json. The
response has a uniform shape: items[] each with structuredContent.variants[]
holding price and stock. Prices are in GBP here.
"""
import requests
import csv
import re
...
BASE = "https://www.watchbrotherslondon.com"
PATH = "/sales-corner"
...
BRANDS = [
    ...
    'Movado', 'Czapek', 'Urwerk', 'Zenith', 'Breguet', 'Grand Seiko',
]
...
def parse_item(i):
    ...
    if variants:
        v = variants[0]
        # Price in minor units (pence). Use priceMoney.value for string in major.
    ...
    return {
        ...
        'img': asset,
        'description': desc,
        'source': 'Watch Brothers London',
        'date': '',  # addedOn is an epoch-ms, we lean on merge.py's firstSeen tracking instead
        'sold': not available,
    }


def main():
    print("Fetching Watch Brothers London inventory (Squarespace)...")
    items = get_all_items()
