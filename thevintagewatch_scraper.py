#!/usr/bin/env python3
"""
The Vintage Watch (thevintage.watch) scraper — Shopify, USD-priced.

Scopes to /collections/available-watches/products.json so the feed stays
clean (the bare /products.json may include archived or sold items).

Run: python3 thevintagewatch_scraper.py
Output: thevintagewatch_listings.csv
"""
...
BASE = "https://thevintage.watch"
COLLECTION_PATH = "/collections/available-watches"
...
def detect_brand(name, vendor=""):
    if vendor and vendor.lower() not in (
        "the vintage watch", "the vintage watch collection", "",
    ):
        for b in BRANDS:
...
def parse_product(p):
    ...
    return {
        ...
        "img": img,
        "description": body,
        "source": "The Vintage Watch",
        "sold": not available,
    }


def main():
    print("Fetching The Vintage Watch inventory (Shopify, available-watches)...")
    ...
    out_file = "thevintagewatch_listings.csv"
