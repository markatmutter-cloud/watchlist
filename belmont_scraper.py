#!/usr/bin/env python3
"""
Belmont Watches scraper - Shopify, limited to the vintage-watches
collection (they also sell new watches — we only want vintage here).
Run: python3 belmont_scraper.py
Requires: pip install requests
Output: belmont_listings.csv
"""
import requests
import csv
import re
...
BASE = "https://www.belmontwatches.com"
...
BRANDS = [
    ...
    'Movado', 'Czapek', 'Urwerk', 'Zenith', 'Breguet', 'Bulova', 'Hamilton',
]


def detect_brand(name, vendor=""):
    if vendor and vendor.strip().lower() not in ['belmont', 'belmont watches', 'pre-owned', '']:
        for b in BRANDS:
...
def main():
    print("Fetching Belmont Watches vintage collection (Shopify)...")
