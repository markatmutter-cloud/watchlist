#!/usr/bin/env python3
"""
Somlo scraper - Shopify public products API, limited to their wristwatches
collection since they also sell pocket watches, jewellery, etc.
Run: python3 somlo_scraper.py
Requires: pip install requests
Output: somlo_listings.csv

Prices are in GBP (London dealer).
"""
import requests
import csv
import re
...
BRANDS = [
    ...
    'Movado', 'Czapek', 'Urwerk', 'Zenith', 'Breguet', 'Girard-Perregaux',
]


def detect_brand(name, vendor=""):
    if vendor and vendor.strip().lower() not in ['somlo', '']:
        for b in BRANDS:
...
def parse_product(p):
    title = p.get('title', '')
    vendor = p.get('vendor', '')
    body = strip_html(p.get('body_html', ''))[:2000]
...
def main():
    print("Fetching Somlo wristwatches inventory (Shopify)...")
