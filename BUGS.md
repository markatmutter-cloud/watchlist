...
### B-99 — Watches of Lancashire frozen since 2026-08-24: the residential move has no liveness signal
- **Reported:** 2026-09-16 (freshness gate, 23d stale) · **Type:** Scrape defect (suspected) + observability gap (certain) · **Severity:** 2 · **Surface:** `scripts/bonhams_residential_scrape.sh`, `watchesoflancashire_scraper.py` · **Status:** Open — needs one check only Mark can run.
- **What is certain.** `data/watchesoflancashire.csv` was last written on **2026-08-24, by a CI "Listings update" commit**. B-81 moved the source to the residential agent on 08-30. Since that move the agent has committed **every ~2 hours** and its commits touch `public/bonhams_lots.json` and `data/bonhams_auctions.csv` **only** — Lancashire's CSV has never once been updated by it. The feed has served the same 54 rows for 23 days.
- **The CI block is real and unchanged**, so moving it back is not the fix: a source-probe from a runner on 2026-09-16 returned Cloudflare's managed challenge (`cf-mitigated: challenge`, "Just a moment...", `_cf_chl_opt`). B-81's diagnosis stands.
- **Two readings, and I cannot settle this one from here.** Either the dealer genuinely has not changed a single item in 23 days (54 items, possible), or the Lancashire leg of the residential script fails every run on Mark's laptop. Both produce an identical git history, because the script's failure path is **silent by design**: on failure it `rm -f`s the partial and leaves the old CSV in place, so `merge.py` keeps emitting the stale rows and nothing pages.
- **The observability gap is real either way, and is the actual bug.** A CI source that stops producing trips the scrape-health gate in 3 runs. A *residential* source that stops producing trips nothing — it is not in the CI move step, so the gate cannot see it — and the only backstop is the 21-day freshness budget. That is a 21-day blind spot on every source we move to the laptop, and we have moved three (Bonhams lots, Bonhams calendar, Lancashire).
- **Proposed fix:** have the residential script write a per-source outcome (`data/residential_status.json`: `{"<source>": {"lastSuccess": "YYYY-MM-DD", "lastError": "..."}}`) and commit it alongside the data, then let a gate fail when a residential source has not succeeded in ~2 days. That converts a 23-day silence into a 2-day alert, and works whichever of the two readings is true.
- **Blocked on:** Mark checking the agent's log on the laptop, or simply whether watchesoflancashire.com's stock has visibly moved since late August.
