...
### B-99 — Watches of Lancashire dead since the day B-81 "fixed" it: relocating cannot beat a JS challenge
- **Reported:** 2026-09-16 (freshness gate, 23d stale) · **Type:** Scrape defect · **Severity:** 2 · **Surface:** `watchesoflancashire_scraper.py`, `cf_clearance.py`, `scripts/bonhams_residential_scrape.sh` · **Status:** Fix written 2026-09-18 (browser escalation) — **unverified until the laptop agent runs it**.
- **Confirmed by Mark 2026-09-16:** the dealer's site is up with **61 listings**. Our feed served the same **54 rows since 2026-08-24**, so the stock moved and we missed all of it.
- **Root cause.** Probes from a runner against the Store API, the homepage and `/wp-sitemap.xml` all returned `cf-mitigated: challenge` with `_cf_chl_opt` `cType: 'managed'`. A Cloudflare **managed challenge** demands JavaScript from every client on every path. It is not an IP block, so B-81's move to the residential laptop could not have helped — that Python scraper is challenged exactly as CI is. Mark sees the site fine because his browser runs the challenge JS. CLAUDE.md already carried this rule, written by B-81 itself.
- **Fix (this PR):** `cf_clearance.BrowserSession` drives headless Chromium via Playwright, waits for `cf_clearance` to be issued, and walks the Store API through the cleared browser context — deliberately fetching *through* the browser rather than harvesting the cookie into another HTTP client, because clearance is bound to the (IP, User-Agent) that earned it and re-sending it elsewhere reintroduces the UA/TLS mismatch that already bit this scraper once. The cheap curl_cffi path still runs first every tick and escalates only on an actual interstitial, so if the zone's posture softens we stop paying for a browser.
- **Also fixed here, a latent data-loss bug found while reading it:** `main()` wrote its CSV unconditionally, so an API answering 200 with an empty list would have produced a header-only file that the agent moved into place, wiping 54 real listings. It now refuses to write on an empty or truncated parse (the caller only moves the file if it exists, so not writing IS what preserves the prior data) and exits non-zero.
- **Residential blind spot, the other half.** A CI source that dies trips the scrape-health gate in 3 runs; a residential one produces nothing that gate can see, leaving only the 21-day freshness budget — which is why this took 23 days. The agent now writes `data/residential_status.json` (`lastRun` / `lastSuccess` / `lastError` per source) and commits it with the data, and the Lancashire leg reports failure loudly instead of printing "transient block?" every hour. **Remaining: a gate that FAILS when a residential source has not succeeded in ~2 days.** That is the piece that turns the file into an alarm, and it is not built yet.
- **Setup Mark must run once** in the residential clone: `pip3 install -r requirements-residential.txt && python3 -m playwright install chromium`. Until then the Lancashire leg fails loudly and records it; the Bonhams legs are untouched.
- **Verification owed.** Neither CI nor this sandbox can reach the dealer, so the browser leg is tested only by the parts around it (challenge detection, status recording, the write guard — 15 new tests). The real proof is one run on the laptop producing a CSV with ~61 rows. Do not call B-99 closed before that.

### B-98 — Chronoholic + ClassicHeuer paged as stale; they are slow by nature, not broken
- **Reported:** 2026-09-09 (freshness gate) · **Type:** Alert tuning · **Severity:** 3 · **Surface:** `source_freshness.py` · **Status:** ✅ Resolved 2026-09-09 — not a scrape defect at all. Per-source content budget, 90d.
- **Detail:** Both produced a CSV every run but their fingerprints had not changed since 2026-08-17/18 (23d and 22d), past the 21-day dealer budget, so the Health report failed daily.
- **Mark's call, and the thing no probe could have told us:** ClassicHeuer is a specialist old-timer, well respected in the watch world but not really a classic retailer. Chronoholic is a collector using the site to rotate their own collection — the stock is exceptional and well respected, it just does not move on a retailer's clock. **Both are valid sources whose stillness is character, not rot.**
- **Fix:** `BUDGET_CHANGED_BY_SOURCE` gives those two a 90-day content budget. Deliberately narrow: `BUDGET_SEEN` is untouched, so "no data at all" still pages in 3 days, and the leash stays finite so a permanently frozen source still surfaces.
- **Worth remembering:** the gate was working correctly and the data was healthy. What was wrong was a default that assumed every dealer is a retailer turning stock over. Before widening a budget, ask which of those two it is — here the answer lived in Mark's head, not in any log.
