...
- **Reported:** 2026-09-16 (freshness gate, 23d stale) · **Type:** Scrape defect · **Severity:** 2 · **Surface:** `scripts/bonhams_residential_scrape.sh`, `watchesoflancashire_scraper.py` · **Status:** Open — root-caused and confirmed; fix needs Mark's call.
- **Confirmed by Mark 2026-09-16:** the dealer's site is up and carrying **61 listings**. Our feed has served the same **54 rows since 2026-08-24**. So the stock has genuinely moved and we have missed all of it — this is not a static dealer.
- **The CSV was last written by CI on 08-24.** B-81 moved the source to the residential agent on 08-30. Since then the agent has committed every ~2 hours, touching `public/bonhams_lots.json` and `data/bonhams_auctions.csv` **only**. It has never once written Lancashire. The source has been dead since the day it was "fixed".
- **Why the fix could never have worked.** Probes from a runner on 2026-09-16 against three different paths — the Store API, the homepage, and `/wp-sitemap.xml` — all return the same thing: `cf-mitigated: challenge`, "Just a moment...", `_cf_chl_opt` with **`cType: 'managed'`**. A Cloudflare **managed challenge** demands JavaScript from *every* client on *every* path. It is not an IP block, so **moving hosts changes nothing** — the laptop's Python scraper gets challenged exactly as CI does. Mark sees the site fine in a browser because his browser runs the challenge JS.
- **CLAUDE.md already said this**, in the entry B-81 itself wrote: "A Cloudflare JS challenge 403s *every* client, so relocating only hides it somewhere quieter; curl_cffi fixes TLS fingerprints, never JavaScript (B-81)." The remedy chosen was the one the rule rules out. The lesson is not the misdiagnosis but the silence: nothing verified that the relocation produced data, and the script's failure path keeps the stale CSV, so it took 23 days and a freshness budget to notice.
- **Options, cheapest first:** (a) **ask the dealer to allowlist us** — Mark has form here (Watch Center, 08-30), dealers generally want their stock listed, and it costs one email; (b) **solve the challenge with a real browser** on the residential agent (Playwright + Chromium, reuse the earned `cf_clearance` cookie for the Store API) — genuine but adds a browser dependency to the laptop; (c) drop the source. Do NOT try curl_cffi or another relocation: both are ruled out by the block type.
- **The observability gap is the other half, and is independent of which option wins.** A CI source that stops producing trips the scrape-health gate in 3 runs; a **residential** source that stops produces nothing the gate can see (it is not in the CI move step), leaving only the 21-day freshness budget. Three sources now run on the laptop. Proposed: the residential script writes `data/residential_status.json` (`{"<source>": {"lastSuccess": "...", "lastError": "..."}}`) and commits it with the data, and a gate fails when a residential source has not succeeded in ~2 days. That turns 23 days of silence into 2.
